...
dtoList만 할일 페이지 업무일 때는 TodoDTO
        ...
        쇼핑 구매 업무일 때는 OrderDTO 등에 list가 필요하므로
...
public class PageResponseDTO <E>{
    ...
    // 다음 페이지의 존재 여부
    private boolean next;   // 이전 페이지가 존재하면 true, prev버튼이 보인다. false면 감춘다.

    private List<E> dtoList;    // 테이블에 보여줘야 할 row 데이터의 list
    ...
    public PageResponseDTO(PageRequestDTO pageRequestDTO, List<E> dtoList, int total){
        ...
        /*
        위에서 end계산은 10 단위로 무조건 맞춘것이기 때문에
        실제 마지막 페이지 번호와 확인해서
